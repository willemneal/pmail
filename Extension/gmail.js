...
//AUSTIN CODE: CHECK VERY CAREFULLY
  api.tools.xhr_watcher = function () {
    if (!api.tracker.xhr_init) {
      ...
      win.XMLHttpRequest.prototype.send = function (body) {
        ...
        var currentEvent;
        var sendingMessageNow = false;
        // fire before events
        if(api.observe.trigger('before', events, this)) {
          currentEvent = events;
          //console.log(events);
          //console.log("event test");
          //console.log(events['http_event'][0]['url']['autosave']);
          
            if (currentEvent.send_message || events['http_event'][0]['url']['autosave'] == 1) {
              sendingMessageNow = true;
              console.log('special event');
              console.log(this.xhrParams.body_params.body);
              globalVariable = this;
              paramsTmp = this.xhrParams.body_params.body;
              kbpgp.box (paramsTmp, function(err, result_string, result_buffer) {
                console.log("in box");
                console.log(err, result_string, result_buffer);

                if(result_string==undefined){
                  globalVariable.xhrParams.body_params.body = paramsTmp;
                }else{
                  globalVariable.xhrParams.body_params.body = '<div dir="ltr">'.concat(result_string).concat("</div>");
                }
                body = arguments[0] = globalVariable.xhrParams.body_is_object ? globalVariable.xhrParams.body_params : $.param(globalVariable.xhrParams.body_params,true);
                var out = globalVariable._gjs_send.apply(globalVariable, arguments);
                api.observe.trigger('on', events, globalVariable);
                return out;
              });
  ...
  */
  ...
  api.dom.email = function(element) {
    ...
    if(  !element.hasClass('adn') ){
      console.log("Bad phrase!");
    }
